...
bool cGLWidget::screenToWorld(int winx, int winy, int z_ndc, QVector3D &pw)
{
    ...
    //qDebug() << "ray clip" << ray_clip << "ray eye" << ray_eye << "mEye" << mEye << rw.normalized();
    //qDebug() << "ray_world" << rw << "point in world" << pw << "rw*t" << rw*t << "t" << t;

    return true;
}

bool cGLWidget::screenToWorld(QVector2D xy_ndc, int z_ndc, QVector3D &pw)
{
    // define the plane
    sPlane plane(QVector3D(0,0,1), QVector3D(0,0,0));
    ...
    QVector4D ray_clip(xy_ndc.x(), xy_ndc.y(), z_ndc, 1.0);
    QVector4D ray_eye = mProjectionMatrix.inverted() * ray_clip;
